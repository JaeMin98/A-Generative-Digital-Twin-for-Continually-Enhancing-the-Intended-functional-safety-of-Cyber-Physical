...
def plot_data(data, ylabel, title, figname):
    ...
    # 원래 데이터 플롯 (연한 색)
    plt.plot(data, label="distance", color="blue", alpha=0.3)

    # EMA 데이터 플롯 (찐한 색)
    ema_data = calculate_ema(data)
    plt.plot(ema_data, label="distance_smooth", color="blue")

    plt.ylabel(ylabel, fontsize=15)
    plt.title(title, fontsize=20)

    plt.xlim([0, len(data)])  # X축의 범위 설정
    plt.xticks(np.arange(0, len(data), 100), fontsize=15)
    plt.ylim([0, 310])  # Y축의 범위 설정
...
plot_data(data[:900], "Distance (m)", "(b) The traveled distance of the ego agent during training", "S1_distance.png")
