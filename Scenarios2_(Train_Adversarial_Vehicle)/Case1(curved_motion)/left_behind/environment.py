...
#----------------------------------------------------------
    ...
    # define state and action space (전진, 회전, 브레이크)
        steering_low, steering_high = -0.3, 0.3
        low = np.array([-11.0, -16.0, steering_low, steering_low, steering_low, 0.5])
        high = np.array([-8.0, -13.0, steering_high, steering_high, steering_high, 1.0])
    ...
    def step(self, action):
        ...
        for i in range( 2 , (2+self.num_of_steering) ):
            ...
            while(1):
                ...
                if(time.time() - Front_car_timer) > 3 :
                    self.set_car_control_of_front_random()
                    Front_car_timer = time.time()

                observations.append(self.observation())
                time.sleep(0.1 * time_scale)
                if (time.time()-start_time) > (3 * time_scale) :
                    break
        ...
        return observations, reward, done, success, self.Is_ROI_Collision
#----------------------------------------------------------
